...
class Vehicles:
    def __init__(self, direction_id, timetil, vehicle_id):
...
for m in (vehiclesdata["data"]):
    ...
    stopid = m["relationships"]["stop"]["data"]["id"]
    myVehicles[index].stop = stops[stopid]
    index += 1

#print to check
for p in myVehicles:
    print("Direction is " + str(p.direction_id))
    print("Arriving in " + str(p.timetil) + " minute(s)")
    print("ID is " + p.vehicle_id)
    print("Vehicle is " + p.current_status)
    print(p.stop)